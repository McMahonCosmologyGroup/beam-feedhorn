...
def beam1d(fre, angle, step):
    ...
    N_PTS = (Y_MAX_ANGLE - Y_MIN_ANGLE) / DELTA_X_Y + 1
    ...
    STR_FILE_OUT = (
        "../Data/" + str(fre) + "GHz_" + str(angle) + "deg_1D_H_" + today + ".txt"
    )
    ...
    def MakeBeamMap(i_f, f, LOs, baseline, fpga):
        ...
        if Y_MIN_ANGLE != 0:
            ser_y_min = ('I2M-' + str(80*abs(Y_MIN_ANGLE)) +',R') 
            ser.write(str.encode(ser_y_min)) #Move motor 1 (our x axis) by X_MIN_ANGLE degrees.
            time.sleep(abs( 0.25*Y_MIN_ANGLE))
        ...
        for y in np.linspace(Y_MIN_ANGLE, Y_MAX_ANGLE, int(N_PTS)):
            ...
            if y < Y_MAX_ANGLE:
                ...
                if abs(DELTA_X_Y) != 0:
                    print('moving x forward')
                    ser.write(str.encode('I2M' + str(abs(80*DELTA_X_Y)) +',R'))   # moves phi-axis 80*DELTA_phi degrees
        ...
        if abs(Y_MAX_ANGLE) != 0: 
            ser_y_max = ('I2M-' + str(80*abs(Y_MAX_ANGLE) ) +',R') #Before this command, y = Y_MIN_ANGLE, so I need to go forward Y_MIN_ANGLE degrees to make it back home. 
            ser.write(str.encode(ser_y_max))
            time.sleep(abs(0.25*abs(Y_MIN_ANGLE)))
            ser.write(str.encode('C')) #Clear all commands from currently selected program. 
            time.sleep(DELTA_T_VELMEX_CMD)
